...
(function() {
  ...
  window.ScatterplotDiscreteGraph = (function(superClass) {
    ...
    ScatterplotDiscreteGraph.prototype.getScaleXDomain = function() {
      console.log('getScaleXDomain', this.data.map((function(_this) {
        return function(d) {
          return d[_this.options.key.x];
        };
      })(this)));
...
(function() {
  (function($) {
    var baseurl, excludedCountries, formatFloat, formatInteger, getCountryName, lang, setVideoMapPolio, setupHeatMapGraph, setupImmunizationCoverageDynamicLineGraph, setupImmunizationCoverageMultipleSmallGraph, setupImmunizationDiseaseBarGraph, setupMeaslesWorldMapGraph, setupVaccineBcgCasesMap, setupVaccineBcgStockoutsMap, setupVaccineConfidenceBarGraph, setupVaccineDiseaseHeatmapGraph, setupWorldCasesMultipleSmallGraph, vaccines;
    lang = $('body').data('lang');
    ...
    if ($('#vaccine-prices').length > 0) {
      ...
      d3.csv(baseurl + '/data/prices-vaccines.csv', function(error, data) {
        var graph;
        data = data.filter(function(d) {
